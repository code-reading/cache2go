...
func TestCache(t *testing.T) {
	...
	table.Add(k+"_1", 0*time.Second, v) //未设置过期时间, 不触发过期检查;
	table.Add(k+"_2", 1*time.Second, v) //会触发过期检查;

	// check if both items are still there
	p, err := table.Value(k + "_1") //err !=nil 表示k 不存在;
	if err != nil || p == nil || p.Data().(string) != v {
		t.Error("Error retrieving non expiring data from cache", err)
	}
	p, err = table.Value(k + "_2") //访问后更新访问次数和时间
	if err != nil || p == nil || p.Data().(string) != v {
...
func TestCacheExpire(t *testing.T) {
	...
	// '1' 还存在, 虽然其生命周期100 ms, 但是上次在75ms时被访问过一次, 所以其生命周期在上次75ms + 100ms = 175ms;
	_, err = table.Value(k + "_1")
	if err != nil {
		t.Error("Error retrieving value from cache:", err)
	}
	...
	//'2'一直未被访问, 其生命周期100ms < 75ms + 75ms 所以'2'过期被删除了;
